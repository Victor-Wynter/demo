...
var MOUTH_TOGGLE_INTERVAL = 100; // Intervalo em milissegundos (500ms = 0.5 segundos)
var lastMouthToggleTime = Date.now(); // Armazena o último momento em que a boca foi alternada

function drawPacman() { 
    var ctx = getPacmanCanevasContext();
    var canvas = ctx.canvas;

    // Limpa o canvas antes de redesenhar o Pacman
    ctx.clearRect(0, 0, canvas.width, canvas.height);

    // Escolhe a imagem correta baseada no estado da boca
    var pacmanImg = (PACMAN_MOUTH_STATE === 0) ? pacmanClosedMouthImg : pacmanOpenMouthImg;

    // Verifica se a imagem já foi carregada antes de desenhar
    if (pacmanImg.complete) {
        ctx.save(); // Salva o estado do contexto

        // Movendo o ponto de origem para o centro do Pacman
        ctx.translate(PACMAN_POSITION_X, PACMAN_POSITION_Y);

        if (PACMAN_DIRECTION === 3) {
            // Espelha a imagem horizontalmente se estiver olhando para a esquerda
            ctx.scale(-1, 1);
        }

        // Girando o canvas de acordo com a direção (sem girar se for para a direita ou esquerda)
        if (PACMAN_DIRECTION === 2) { 
            ctx.rotate(Math.PI / 2); // Para baixo
        } else if (PACMAN_DIRECTION === 4) { 
            ctx.rotate(-Math.PI / 2); // Para cima
        }

        // Desenha a imagem centralizada no novo ponto de origem
        ctx.drawImage(pacmanImg, -PACMAN_SIZE, -PACMAN_SIZE, PACMAN_SIZE * 2.3, PACMAN_SIZE * 2.3);
    ...
    // Alterna o estado da boca a cada MOUTH_TOGGLE_INTERVAL milissegundos
    var currentTime = Date.now();
    if (currentTime - lastMouthToggleTime >= MOUTH_TOGGLE_INTERVAL) {
        PACMAN_MOUTH_STATE = 1 - PACMAN_MOUTH_STATE;
...
// Certifique-se de que as imagens sejam carregadas antes de iniciar o jogo
pacmanOpenMouthImg.onload = pacmanClosedMouthImg.onload = function() {
    console.log('Imagens carregadas, agora você pode iniciar o jogo');
};
